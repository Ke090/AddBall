...
describe('physics safety and collisions', () => {
  ...
  it('reflects a ball elastically at a wall', () => {
    ...
    expect(Vector.magnitude(ball.body.velocity)).toBeCloseTo(1, 10);
  });

  it('preserves a single large ball speed across repeated wall bounces with zero friction and gravity', () => {
    const world = new PhysicsWorld({ ...settings, friction: 0, mergeProbability: 0, splitProbability: 0 }, () => 0.5);
    const [ball, ...removed] = [...world.balls.values()];
    for (const other of removed) {
      Composite.remove(world.engine.world, other.body);
      world.balls.delete(other.id);
    }
    const targetRadius = Math.sqrt(C.TOTAL_AREA / Math.PI);
    Body.scale(ball.body, targetRadius / ball.body.circleRadius!, targetRadius / ball.body.circleRadius!);
    ball.area = C.TOTAL_AREA;
    Body.setPosition(ball.body, { x: C.FIELD_SIZE / 2, y: C.FIELD_SIZE / 2 });
    Body.setVelocity(ball.body, { x: 1, y: 0.31 });
    const initialSpeed = Vector.magnitude(ball.body.velocity);

    for (let frame = 0; frame < 360; frame++) world.step(16, noGravity);

    expect(Vector.magnitude(ball.body.velocity)).toBeCloseTo(initialSpeed, 8);
    expect(ball.body.position.x).toBeGreaterThanOrEqual(targetRadius);
    expect(ball.body.position.x).toBeLessThanOrEqual(C.FIELD_SIZE - targetRadius);
    expect(ball.body.position.y).toBeGreaterThanOrEqual(targetRadius);
    expect(ball.body.position.y).toBeLessThanOrEqual(C.FIELD_SIZE - targetRadius);
  });
