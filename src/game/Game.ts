...
export class Game {
  readonly settings=new SettingsStore(); readonly world=new PhysicsWorld(this.settings.values); readonly renderer:CanvasRenderer; readonly tilt=new TiltController(); readonly audio=new GameAudio();
  ...
  private frame=(now:number) => { if(!this.running)return; const delta=Math.min(now-this.last,C.MAX_DELTA_MS); this.last=now; const gravity=this.tilt.gravity(); if(!document.hidden)this.world.step(delta,gravity); this.renderer.render(now); this.fps+=(1000/Math.max(delta,1)-this.fps)*.08; if(now-this.statusLast>100){this.statusLast=now;this.updateTiltStatus();} if(this.debug&&now-this.debugLast>250){this.debugLast=now;const area=totalArea([...this.world.balls.values()].map(b=>b.area)),expected=this.settings.values.initialBallCount;this.debug.textContent=`FPS ${this.fps.toFixed(0)}\n玉 ${this.world.balls.size}\n総面積 ${area} / ${expected}\n重力 ${gravity.x.toFixed(4)}, ${gravity.y.toFixed(4)}\n合体 ${(this.settings.values.mergeProbability*100).toFixed(0)}%\n分裂 ${(this.settings.values.splitProbability*100).toFixed(0)}%`;this.debug.classList.toggle('warning',area!==expected);} requestAnimationFrame(this.frame); };
}
